...
#if defined (NEED_ISHFT)
#if defined(FSYMSFX) && (FSYMSFX!=0)
#define ishft       ishft_
#define iand        iand_
#define ior         ior_
#endif
...
*/
	...
	if( *ipMode == 0 ) /* read the named directory */
	{
		...
		*cpDelim = '\0'; 
		...
		if( FileFind( caPath, xxx, AV("bdj") ) <= 0 ) /*
			Unfriendly behavior removed - GRH/UB
			exit(1) */ ;
		cBeginFlag = FALSE;
	}
	(void)memset( caTemp, '\0', BUFLEN);
	...
	if( strstr(caStat, "*") != NULL ) {
		...
		iWildCardAll = 1;
        	...
        	char *c = strrchr( caName, '/' );
#ifdef DEBUG   
        fprintf( stderr, "\n#==> caName %s",caName );
#endif
        	if (c) (void)strcpy( caTemp, c+1 ); else caTemp[0] = '\0';
...
/*	} */
	...
	for( iCount = strlen(caTempBuf); iCount < 14; iCount++ )
		caTempBuf[iCount] = '0';
	strcat( caTempBuf , ".0" );
	{ int iyy, imo, idy, ihh, imm, iss;
	   if (6 != sscanf(caTempBuf,"%4d%2d%2d%2d%2d%2d",
	      &iyy,&imo,&idy,&ihh,&imm,&iss))
	      fprintf(stderr, "**ExtractTime: Invalid time %s\n",caTempBuf);
	   idy = julday(imo,idy,iyy) - julday(1,0,iyy);
	   dBackTime = (double)(1000*iyy + idy) + (double)((ihh*60 + imm)*60 + iss)/86400.;
        }
...
/* JULDAY -- Return Julian day given month, day, year

      Transliterated from Fortran by G. Helffrich/UB

*/
int julday(imm,idd,iyyy)
   int imm, idd, iyyy;
{
      ...
      if (iyyy<0) iyyy+=1;
      if (imm>2) { jy=iyyy; jm=imm+1;} else { jy=iyyy-1; jm=imm+13;};
      jday = (int)(365.25*jy)+(int)(30.6001*jm)+idd+1720995;
      if (idd+31*(imm+12*iyyy)>=IGREG) {
        ja=(int)(0.01*jy);
...
int    iCount = 0;
	...
	if( cpTemChar2 && (strlen(caTempBuf2) - strlen(cpTemChar2)) > 12 )
		return( TRUE );
	else
		return( FALSE );
...
double dBackTime;
	...
	{ int iyy, imo, idy, ihh, imm, iss;
	   if (6 != sscanf(caTempBuf,"%4d%2d%2d%2d%2d%2d",
	      &iyy,&imo,&idy,&ihh,&imm,&iss))
	      fprintf(stderr, "**ToLongTime: Invalid time %s\n",caTempBuf);
	   idy = julday(imo,idy,iyy) - julday(1,0,iyy);
	   dBackTime = (double)(1000*iyy + idy) + (double)((ihh*60 + imm)*60 + iss)/86400.;
        }
...
/* ======================================================================== */
/* Change time from seconds since 1 Jan, 1970 to internal format            */
/*   yyyyddd.sssss - yyyy year, ddd julian day, ssss second in day          */
...
/* ======================================================================== */
   double	itime;
{
   int iyear, isecyr, isecdy, iday;

   iyear=itime/31536000+1970;
   for(;;) {
      isecyr=(iyear-1970)*31536000+((iyear-1969)/4)*86400;
      if(isecyr <= itime) break;
      iyear-=1;
   };
   iday=(itime-isecyr)/86400+1;
   isecdy=(iday-1)*86400;
   return((double)(iyear*1000 + iday) + (double)(isecdy)/1e5);
}

/* ======================================================================== */
/* Change time from yyyyddd.sssss to seconds since 1 Jan, 1970              */
/*   G. Helffrich/U. Bristol                                                */
/* ======================================================================== */
double FromStd( itime )
/* ======================================================================== */
   double	itime;
{
   return(
      (double)(((int)(itime/1000.)-1970)*31536000.0) /* years elapsed */
    + (double)86400.0*(((int)itime)%1000)            /* days elapsed */
    + 1e5*(itime-(double)((int)itime))               /* seconds elapsed */
    );
}

/* ======================================================================== */
/* Change time from seconds since 1 Jan, 1970 to yyyymmddhhmmss.0 form to   */
/*   yyyyddd.sssss - yyyy year, ddd julian day, ssss second in day          */
/*   G. Helffrich/U. Bristol                                                */
/* ======================================================================== */
double ToLongDouble( itime )
/* ======================================================================== */
   double	itime;
{
   int iyear, imm, iday, yymmdd, hhmmss;

   yymmdd = (int)(itime/1e6); hhmmss = itime - 1e6*(double)yymmdd;
   iyear = yymmdd/10000; iday = yymmdd%100; imm = (yymmdd/100)%100;
   iday = julday(imm,iday,iyear) - julday(1,0,iyear);
   return((double)(iyear*1000 + iday) +
      (double)(hhmmss%100 + 60*((hhmmss/100)%100 + 60*(hhmmss/10000)))/86400.);
}
