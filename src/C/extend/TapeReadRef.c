...
#endif
	if( lseek (fdTape, *offset, SEEK_CUR) != fdTape ) {
...
#ifndef LINUX
	if( strncmp(caTmpName,"rsd", 3) == 0 ) {
        	if (!cklabel(caDevName, 0)) {
                	fprintf(stderr, "ERROR checking disk against kernel's label\n" );             
                	exit(1);
		}
        }
#endif
...
*/
	...
	{
#ifdef DEBUG
  		fprintf( stderr, "\n#READ_ERR <c_tread> read blocks (%d + %d)\n",iGBlocks
							   ,iPBlocks );
#endif
		iNewFile = FALSE;
		return( (int)READ_ERR );
