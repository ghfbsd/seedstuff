	function read_grf(unit,idir,mode,istim,irec,jbuf,iftim,iftsc,
...
c
	...
	data stat_grf/'GRA1','GRA1','GRA1','GRA2','GRA3','GRA4','GRB1',
     &  'GRB1', 'GRB1','GRB2','GRB3','GRB4','GRB5','GRC1','GRC1',
     &  'GRC1','GRC2', 'GRC3','GRC4'/
	data chan_grf/'BHZ','BHN','BHE','BHZ','BHZ','BHZ','BHZ','BHN',
     &  'BHE', 'BHZ','BHZ','BHZ','BHZ','BHZ','BHN','BHE','BHZ','BHZ',
     &  'BHZ'/
...
c
	 if(ird.eq.1) then
	  call rdisc(unit,ibuf,irtim,idir,mode,0,nch,kan,idt,ilen,
     &      iftm,ichk)
	  mode=2
	  iftim=iftm*10
	  iftsc=0
...
c
	...
	data ks/19/,nsamp/200/,inhead/40/,ii/1/
C
	ichi=ichk
	ichk=0
	itim=iblck
...
c	if(ichr.eq.2.and.(mode.eq.2.or.nrec.lt.nrec_s)) goto 110
	if(ichr.eq.2.and.(mode.eq.2.or.(nrec.lt.nrec_s.and.nrd.gt.2)))
     &    goto 110
...
c	  indxa=index(new_file,'grf')
	...
	if(iopen.eq.0.or.indx1.eq.0) then
	  ...
	  open(idsc,file=new_file,status='old',form='unformatted',
...
c
	integer byteorder
	integer*2 ibuf(*),ibfix(1024),iprec(512),ipanf(512),
     &	jpoint(1024)
        logical swap
	external ltime
	data nrect/0/, nndat/0/
c
c
...
c
	if(nrec.lt.2) then
	  ...
	  if (swap) call fiswap(nwfix,ibfix,ibfix)	! swap for SUN only
	  ndat=ibfix(2)
	  if(ndat.eq.500) then
	    do i=1,ndat
	      ibuf(i)=ibfix(i+1)
...
c
	    ...
	    if(iend.ne.0) then
	      ...
	      if (swap) call fiswap(nwfix,jpoint,jpoint) ! swap for SUN only
	      ipoint=1
	      do i=1,nwfix/2
	        iii=2*i
	        ii=iii-1
...
c		      ichk=20
	              if(ichi.ge.0)
     &                  write(*,*) 'Warning: 2 pointer identical:',
...
99	      format(1x,a,i4,1h/,i2,1h/,2i4,2(1h:,i2))
	    ...
	    ichk=-2
	    if(ichi.ge.0)
     &        write(*,*) 'Warning: No Header found',(ibfix(i),i=1,5)
	    icha=ichi
...
880	ichk=4
	...
	if(ichi.ge.0) then
     ...
     &      ipoint,mrec,nlast,inrec,inanf
	    ...
	    call abszeit(0,last,iyear,iday,ih,im,is,icha)
	    call sub_datum(0,iday,iyear,mon,itag)
	    write(*,'(1x,a,i4,1h/,i2,1h/,2i4,2(1h:,i2))')
     &        'Last correct time:',itag,mon,iyear,ih,im,is
	endif
	return
c
	...
	do j=2,nwfix
	  ...
	  if(n.eq.ndat) then
	    ...
	    if(irec.ne.irecp.or.ianf.ne.ianfp) then
	      ndatk=ibfix(ianfp)
	      if(ichk.ge.0)
     &          write(*,*) 'Pointer check error',mrec,irec,ianf,ndat,
     &	          irecp,ianfp,ndatk
	      ichk=1
	    endif
	    npoint=npoint+1
...
100	itsec=ih*360+im*6+is/10
	if(iyear.lt.1970.or.iyear.gt.2000.or.iday.lt.1.or.iday.gt.366
     &	.or.itsec.lt.0.or.itsec.gt.8640) then
	  ichk=1
	  if(ichi.ge.0)
     &      write(*,*) 'ABSZEIT: wrong input time',itsec,iday,iyear
	  nerr=nerr+1
...
C
C     ***************************************************************
      INTEGER*2 ZFF00,Z1E,Z80,ZFF,ZF000
      PARAMETER (Z1E=16+14, Z80=8*16, ZFF=255)
      INTEGER*2 IBUF,JDAT,ITEMP,ITEM1,ITEM2,IVOR,I15
      DIMENSION JDAT(3840),IBUF(*)
      LOGICAL FLAG
      DATA ITEMP/0/,I15/15/, ZFF00/Z'FF00'/, ZF000/Z'F000'/
c
	ichi=ichk
	ichk=0
C
...
c---Beginn Hauptschleife 100
	...
	if(mspur.gt.nspur) then
	  ichk = 11
	  if(ichi.ge.0)
     &      write(*,*) ' Zu viele Spuren:',mspur,(ibuf(i),i=3,4)
	  goto 999
	endif
20    CONTINUE
c---Abfrage auf 0e (Beginn einer Komprimiersequenz) sonst Kopieren des Wertes
      IF (FLAG) GO TO 200
c	write(*,'(a,3z6)') 'flag',flag,JDAT(ii),IAND(JDAT(ii),I15)
      IF (IAND(JDAT(ii),I15) .EQ. 14) GO TO 200
c--Unkomprimierte Daten
      M = LHEAD + MSPUR + MSAMP*NSPUR
	if(m.gt.ilen) then
	  ichk = 12
	  if(ichi.ge.0)
     &      write(*,*) ' Daten-Ueberlauf',m,mspur,msamp,(ibuf(i),i=3,4)
	  goto 999
	endif
      IBUF(M) = JDAT(ii)
c---Zwischenspeichern des letzten Wertes beim Kopieren
	...
	if(msamp.gt.nsamp) then
	  ichk = 13
	  if(ichi.ge.0)
     &      write(*,*) ' Msamp groesser 200:',msamp,mspur,
     &        (ibuf(i),i=3,4)
	  goto 999
	endif
	goto 80
...
80    CONTINUE
100   CONTINUE
      if(lhead.gt.5) IBUF(5) = ABS(IBUF(5))
999   continue
      RETURN
      END
...
c
	...
	INTEGER*4 ISUM,MANT2,ior,iand
	EQUIVALENCE (ISD(1),ISUM)
	DATA MFFF0/Z'FFF0'/, I15/15/
C
	DO 100 K=1,NCH
	L=0
	DO 30 J=1,NDAT,IDT
...
c
	...
	write(*,*) 'next_grffile found: ',ffile
		nfrec=360
        if(ichn.le.0) then
          open(idsc,file=ffile,status='old',form='unformatted',
