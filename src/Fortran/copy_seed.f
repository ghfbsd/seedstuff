...
C       Program to copy slices of data from an archive of mseed files.
C       The archive is assumed to have mseed data in big-endian order.
C       However, when the data is in memory, it is swapped into the native
C       endian order, processed (time cutting), and then swapped back out
C       to big-endian order before writing.  This leads to some strange-looking
C       internal program logic, and there is significant vestigial code to
C       swap at intermediate stages.

C       In order to optimize handling, when an mseed block is found in the
C       archive in which a cut begins, the data is copied from the cut begin
C       into the beginning of a new block.  The rest of the cut data comprises
C       the first block -- the rest of the new block is just zero.  Then any
C       intermediate blocks are copied wholesale from the archive to the output
C       volume.  When the block with the end of the cut is found, only as much
C       data as needed to satisfy the cut fills the output block; the rest is
C       zero.  This saves decoding, shifting and encoding potentially a lot of
C       data.  This behavior is flagged by fast=1.  If the slower but more
C       compact volumes are desired, set fast=0.

C       Code originally by Winfried Hanka, GFZ, circa 1995.
C       Significantly modified and sparsely documented by
C          G. Helffrich/U. Bristol 1995-2007.
...
c     &	,kbuf(4000)
	...
	real*8 first_tim,dum_tim,dua_tim,due_tim,new_tim,next_tim,
     &  next_timw,last_tim,sta_stim,sta_etim,
     &  lst_tim,wra_tim,wre_tim,rnd_tim,first_dtim,last_dtim
c
	character files(300)*132
	integer sera_idx(100,100,15),sere_idx(100,100,15),
     &    stat_lkup(100),no_chan(100),chan_lkup(100,15),nser(100,15),
     &    no_rec(300)
	real rateo(100,15)
	real*8 spa_tim(100),spe_tim(100),sera_tim(100,100,15),
     &    sere_tim(100,100,15),vol_anf,vol_end
c
	parameter (unit=1,unit1=10,unit2=2,unit3=3)
	parameter (unit4=4,unit5=7,unit6=8)
...
c
	...
	data ianf/0/, totsmp/0/, totsmp_1/0/
	data new_tim/0/, first_dtim/0/, last_dtim/0/
c
...
c
	...
	if (istat.ne.1) stop '**No input data directory (-i) provided'
	ln=lenc(idir)
	if(idir(ln:ln).ne.'/') idir(ln+1:ln+1)='/'
	istat=get_opt('o',odir,nopt,opt,opt_string)
	if (istat.ne.1) stop '**No output data file (-o) provided'
	ln=lenc(odir)
	if(odir(ln:ln).ne.'/') odir(ln+1:ln+1)='/'
	istat=get_opt('b',start_string,nopt,opt,opt_string)
	if (istat.ne.1) stop '**No begin time (-b) provided'
	call change_time_string(start_string,itag,mon,iyear,ih,im,is)
	istat=get_opt('e',end_string,nopt,opt,opt_string)
	if (istat.ne.1) stop '**No end time (-e) provided'
	call change_time_string(end_string,itagl,monl,iyearl,ihl,iml,
     &                          isl)
	...
	do i=1,mx_opt
	  optarr(i)=.false.
        enddo
	if(1.eq.get_opt('t',label,nopt,opt,opt_string)) then
c         Trace option letters: p - parse, h - seed header generation
c            t - start time, d - mseed data reblocking
          optpar=0.ne.index(label,'p')
          optshd=0.ne.index(label,'h')
          opttim=0.ne.index(label,'t')
          optdat=0.ne.index(label,'d')
	endif
c
...
c
c	call parse_seed_db(11,12,cfgfile,' ',' ')
	call parse_seed_db(11,12,cfgfile,stat_sel,net_sel)
c
	write(*,*) 'Network(s) selected:',no_net,' ',
     &    (net(i),i=1,no_net)
...
500	if(idiv.eq.1) then
	  ...
	  if(iday.eq.1) then
	    istim=first_dtim
	    idyl=idy+1
	    call datum(0,idyl,iyear,monl,itagl)
	    ihl=0
	    iml=0
	    isl=0
	    call abstim(1,iltim,iyearl,idyl,ihl,iml,isl)
	  ...
	  call abstim(0,istim,iyear,idy,ih,im,is)
	  call datum(0,idy,iyear,mon,itag)
	  call abstim(0,iltim,iyearl,idyl,ihl,iml,isl)
	  call datum(0,idyl,iyearl,monl,itagl)
	  write(*,*) 'Process day',iday,itag,mon,ih,im,is,
     &                itagl,monl,ihl,iml,isl
	endif
c	
	ihead=1
	jred=0
...
c	        write(*,*) 'n',n,stat_net(n),stat_end_tim(n)
                call parse_time(stat_st_tim(n),sta_stim)
                call parse_time(stat_end_tim(n),sta_etim)
		lopen = sta_etim.gt.first_tim.and.sta_stim.lt.last_tim
	        if(stat_net(n)(1:1).ne.' '.and.lopen) then
	          no_stat=no_stat+1
...
c
	    ...
	    if(0 .eq. index('ESHBLVUM',chan_typ)) then
...
c	  call abstim(0,istim,iyear,idy,ih,im,is)
c	  call datum(0,idy,iyear,mon,itag)
c
	endif
c
...
c     &	last_tim,jred,more
        if (opttim) then
	  call abstim(0,int(first_tim),iyrf,idyf,ihf,imf,isf)
          call datum(0,idyf,iyrf,monf,itgf)
	  write(*,9) 'Start time wanted:',
     &	    ' ',' ',0.0,itgf,monf,iyrf,ihf,imf,isf,0
	  call abstim(0,int(last_tim),iyrf,idyf,ihf,imf,isf)
          call datum(0,idyf,iyrf,monf,itgf)
	  write(*,9) 'End time wanted:',
     &	    ' ',' ',0.0,itgf,monf,iyrf,ihf,imf,isf,0
        endif
	if(jred.eq.1) fast=0
	imode=0
...
200	istat=open_next(imode,unit,idir,first_tim,last_tim,chan_typ,
     &                  ifile,nlgfl)
c	write(*,*) 'openn',istat,ifile,nlgfl
	...
	ichk=check_infile(ifile,no_stat,sr_stat,no_comp,sr_chan,istn,
     &    ichn)
	...
	if(icp.lt.1) then
     ...
     &	  neof,lst_tim,jred,ibuf)
	    ...
	    call abstim(0,iftim,iyrf,idyf,ihf,imf,isf)
            call datum(0,idyf,iyrf,monf,itgf)
	    write(*,9) 'File time in '//ifile(1:index(ifile,' ')),
...
1010	  do while (istat.eq.0.and.last_tim-next_tim.gt.dt)
	    irec=irec+1
	    istat=read_rec_fil('QT',1,unit,nlgfl,1024,ibuf,irec,
     &         num,form,stat,chan,net_id,iftim,iftsc,timcr,rate,nsamp,
     &         iof,nframe)
	    if (nframe.gt.63) then
	       write(*,*) '**Bad frame value: ',nframe,'; in rec ',
     &           irec
	    endif
	    if (optdat) then
	       write(*,9001) irec,form,stat(1:lenc(stat)),chan,
     &                       nframe,nsamp
9001           format('*d rec ',i5,' format ',i4,1x,a,1x,a,
     &                ' frames ',i2,' samples ',i5)
	    endif
...
9	        format(3(1x,a),f5.1,1x,2(i2.2,1h/),
     &                 i4.4,2x,2(i2.2,1h:),i2.2,1h.,i4.4)
	        ...
	        call abstim(0,iftim,iyrf,idyf,ihf,imf,isf)
	        call datum(0,idyf,iyrf,monf,itgf)
	        if(ihf.eq.0 .and. imf.eq.0 .and. isf.eq.0 .and.
     &             iftsc.eq.0) then
...
c	        new_recl = 1024 ! only with out the -xl flag (1024*4)
	        ...
	        open(unit1,file=ofile,status='unknown',recl=new_recl,
...
c	      if(last_tim-new_tim.gt.0.5*dt) then
	      if(last_tim-new_tim.gt.dt) then
	        ifchk=istat
	        if(fast.eq.0 .or. nrec.eq.1 .or.
     &            last_tim-next_tim.lt.dt) then
	          idchk=decomp_steim(form,ifchk,nrec,nframe,nsamp,
     &              ibuf(iof+1),ni_dat,jbuf,last_val)
...
c	write(*,*) 'istart 1:',ismp_st,nsamp,ni_dat,new_tim
	          if(ismp_st.eq.2.and.fast.eq.1) then
	            write(*,*)
     &                'Start index corrected to 1 in first record'
	            ismp_st=1
	          endif
...
c	write(*,*) 'wr-seed',nrec,numw(nch),isamp,ichwk
	          ...
	          call corr_tim(wra_tim,timcr,dtw,numw(nch),
     &                          stat,chanw,net_idw,iof,isamp,
     &                          form,12,ibuf)
	          wre_tim=wra_tim+(isamp-1)*dtw
	            ...
	            if(nrec.eq.1) then
	              ...
	              mdat=0
	              if(timcr.lt.0) then
	                call tfix(wra_tim-dtw,iftim,iftsc)
	              else
	                call tfix(wra_tim,iftim,iftsc)
	              endif
	              call abstim(0,iftim,iyrf,idyf,ihf,imf,isf)
	              call datum(0,idyf,iyrf,monf,itgf)
	              write(*,29) 'Start of first output record:',
     &	                itgf,monf,iyrf,ihf,imf,isf,iftsc,timcr
29	              format(1x,a,9x,2(i2.2,1h/),i4.4,2x,2(i2.2,1h:),
     &                       i2.2,1h.,i4.4,i5)
c	            elseif(last_tim-next_tim.lt.0.5*dt) then
	            elseif(last_tim-next_tim.lt.dt) then
	              ia=1
	              if(timcr.lt.0) then
	                call tfix(wre_tim,ietim,ietsc)
	              else
	                call tfix(wre_tim+dtw,ietim,ietsc)
	              endif
     ...
     &	              itge,mone,iyre,ihe,ime,ise,ietsc,timcr
	              ...
	              write(*,*)
     &                  'Error: copy start and end in same record'
	            endif
		    if (optdat) then
		      write(*,9002) nrec,isamp
9002                  format('*d rec ',i5,' compress ',i5,' bytes')
		    endif
	            mode=0
...
c	write(*,*) 'compress cps:',mode,nrec,isamp,ndat,mdat,totsmp,totsmp_1
	            if(ndat.lt.1024 .or. 
     &                 (ndat.eq.1024 .and. totsmp_1.lt.totsmp)) then
		      if (optdat) then
		        write(*,9003) nrec
9003                    format('*d rec ',i5,' flush compressed data')
		      endif
	              icmp=compress_steim(999,1,nrec,0,jbuf(ia),iof,
     &	              1024,ndat,mdat,ist_out,ibuf,totsmp,totsmp_1)
	            elseif(ndat.gt.1024) then
	              write(*,*) 'Compress error: too much data',nrec,
     &                  ia,isamp,ndat,mdat,icmp,icdf,iftim,iftsc,timcr
	            endif
	            call tfix(wra_tim,iftim,iftsc)
	            icdf=def_fixhead(numw(nch),stat,chanw,net_idw,dtw,
     &                iof*4,iftim,iftsc,timcr,mdat,10,12,ibuf)
	            if(mdat.eq.0) then
	              write(*,*) 'Compress error: zero counter',nrec,
     &	                ia,isamp,ndat,mdat,icmp,icdf,iftim,iftsc,timcr
	            endif
	          endif
	          numw(nch)=numw(nch)+1
...
c
	          if(irec.eq.1.and.nrec.gt.1)
     &               call del_overlap(unit1,irec,wra_tim,next_timw,
     &                                dtw,isamp)
c
	          call uncorr_tim(ibuf,net_idw)
c
	          next_timw=wre_tim+dtw
		  call getsbe(ibuf,frst_val,last_val)
		  if (optdat) then
		    write(*,9004) nrec,(ibuf(12+i),i=0,3)
9004                format('*d rec ',i5,' (pre-swap) block 1000:',
     &                     4(1x,z8.8))
		  endif
...
c	write(*,*) 'check_rec 1:',chmod,nch,nfiles,numw(nch),nst,timcr,nser(1,nch),nrec
	            call check_rec(chmod,nfiles,numw(nch),stat,chanw,
     &                ratew,isamp,wra_tim,timcr,nst,stat_lkup,no_chan,
...
c     &	no_chan(nst)),nspn,nser(nch)
	  ...
	  if(istat.eq.1) then
	    imode=1
	    istat=open_next(
     &        imode,unit,idir,next_tim,last_tim,chan_typ,ifile,nlgfl
     &      )
	    if(istat.eq.0) then
	      irec=0
	      goto 1010
...
c	write(*,*) 'check_rec 2:',chmod,numw(nch),nrec,istat,wra_tim,timcr,isamp
	      call check_rec(chmod,nfiles,numw(nch),stat,chanw,ratew,
     &          isamp,wra_tim,timcr,nst,stat_lkup,no_chan,chan_lkup,
     &          rateo,vol_anf,vol_end,nspn,spa_tim,spe_tim,nser,
     &          sera_tim,sere_tim,sera_idx,sere_idx)
	    endif
	  endif
	endif
	istat=0
...
900	if(ihead.eq.1) then
     ...
     &	        net(inet)(1:lenc(net(inet))),cyear(iyear),mon,itag,ih,
     &          im,is,'.DSC'
	      endif
c
	      if(.not.lopen)
     &	      open(unit5,file=ofile,status='unknown')
	    endif
c
	    if(imul.eq.0) then
...
c     &	      sera_idx,sere_tim,sere_idx,nfiles,files,no_rec)
     ...
     &	        ih,im,is,'.LST'
	      ...
	      ix=index(ofile,'.LST')
	      seed_lst=ofile
	      seed_out=ofile(1:ix)//'SEED'
	      seed_idx=ofile(1:ix)//'IDX'
c	write(*,*) imul,lopen,ofile,seed_lst,seed_out,seed_idx
c
	      if(.not.lopen)
     &	      open(unit3,file=ofile,status='unknown')
c
	      if(ih.eq.0.and.im.eq.0.and.is.eq.0) then
	        write(ofile,'(2a,3i2.2,a)') odir(1:lenc(odir)),
     &	        net(inet)(1:lenc(net(inet))),cyear(iyear),mon,itag,
...
c
	      ...
	      if(.not.lopen) then
c     &	      open(unit4,file=ofile,status='unknown',form=
c     &	      'unformatted',recl=1024)
c	        open(unit4,file=ofile,status='unknown',access='direct',form=
c     &	        'unformatted',recl=4096)
c     &	      'unformatted',recl=1024)
	        open(unit4,file=ofile,status='unknown')
	      endif
c	      
cc	      nfiln=lenc(ofile)
...
c     &	  chan_lkup)
     ...
     &	    sera_idx,sere_tim,sere_idx,id_short,'N',ird_shd,unit5,
     &      unit4,unit6,odir,cfg_dir)
	  close(unit3)
	  close(unit4)
  	  close(unit5)
	endif
...
c	
	if(id_keep.eq.1) then
	  open(unit1,file=seed_idx,status='unknown')
	  write(unit1,*) 'vol',vol_anf,vol_end,' nst',nst
	  do i=1,nst
	    n=stat_lkup(i)
	    write(unit1,*) i,' stat_lkup',stat_lkup(i),
     &        ' no_chan',no_chan(n)
	    do j=1,no_chan(n)
	      m=chan_lkup(n,j)
	      write(unit1,*) n,j,' chan_lkup',chan_lkup(n,j),
     &          ' rateo',rateo(n,m),' nser',nser(n,m)
	      do k=1,nser(n,m)
	        write(unit1,*) k,n,m,
     &            ' series',sera_idx(k,n,m),sera_tim(k,n,m),
...
8000	if(id_mero.eq.1) then
	  ...
	  seed_out=ofile(1:i)//'SEED'
	  seed_idx=ofile(1:i)//'IDX'
	  open(unit1,file=seed_idx,status='old')
	  ...
	  do i=1,nst
	    ...
	    do j=1,no_chan(n)
	      m=chan_lkup(n,j)
	      read(unit1,*) n,jj,cdum,chan_lkup(n,j),cdum,rateo(n,m),
     &          cdum,nser(n,m)
	      do k=1,nser(n,m)
	        read(unit1,*) kk,n,m,cdum,sera_idx(k,n,m),
     &            sera_tim(k,n,m),sere_idx(k,n,m),sere_tim(k,n,m)
	      enddo
	    enddo
	  enddo
...
c
	call ieee_flags('clearall',dummy1,dummy2,dummy3)
	end
	subroutine copy_seed_use
c
1000    format('Extracts and assembles MiniSEED data to SEED & files',
     &  /,'Usage:'
     &  /,'copy_seed -b <start_time> -e <end_time> ',
     &    '-[a,b,e,h,i,l,o,s,t]',
     &  /,'A database file copy_seed.cfg has to be present in ',
     &    '$SEED_STUFF_HOME or',
     &  /,'  -a <filename> - alternate database file',
     &  /,'  -b <yymmdd_hhmmss> - start time',
     &  /,'  -c <code> - channel code (def a)',
     &  /,'             h : VSP (HH?, 80 Hz)',
     &  /,'             m : MP  (MH?, 40 Hz)',
     &  /,'             b : VBB (BH?, 20 Hz)',
     &  /,'             l : LP  (LH?, 1 Hz)',
     ...
     &  )
1001    format(
     &    '  -d - separation in day SEED volumes',
     &  /,'  -e <yymmdd_hhmmss> - end time',
     &  /,'  -h <code> - header code',
     &  /,'             f : full SEED header (def)',
     &  /,'             a : abbreviated header (no FIR filters)',
     &  /,'             s : use stored header (in $SEED_STUFF_HOME)',
     &  /,'  -i <idir> - indir  (def=./)',
     &  /,'  -k - keep meta files and header DB (def: no)',
     &  /,'  -l <label> - volume label',
     &  /,'  -m <mergefile> - merge only, file list from <mergefile>',
     &  /,'  -n <netcode> - select one network only',
     &  /,'  -o <odir> - outdir  (def=../)',
     &  /,'  -s <statcode> - select one station only',
     &  /,'  -t <codes> - trace activity selected by code(s):',
     &  /,'             d : mseed data reblocking',
     &  /,'             h : seed header generation',
     &  /,'             p : database parse',
     &  /,'             t : time stamps in files opened'
     &  )
c
        write(*,1000)
        write(*,1001)
