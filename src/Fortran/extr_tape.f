...
2000	if(iselis.eq.1) then
     ...
     &      end_string(1:lenc(end_string))
	  istat=change_time_string(
     &      start_string,itag,mon,iyear,ih,im,is
     &    )
	  call datum(1,idy,iyear,mon,itag)
	  call abstim(1,istim,iyear,idy,ih,im,is)
	  start_tim=dble(istim)
...
c
	write(*,*) 'Extracts Quanterra station tapes and Reftek ',
     &    'disks/tapes'
	write(*,*) 'Usage:'
	...
	write(*,*) '  -D - Reftek disk drive (for random jump - Linux ',
     &    'version only)'
	write(*,*) '  -S <new_statcode> - change station code (for ',
     &    'RF only)'
c
	stop
	end
