...
cc	lenr=1024
	lenr=nbtin
	nrep=1
	open(idsc1,file=seed_in,status='old',form='unformatted',
     &    access='direct',recl=lenr)
	seed_out=odir(1:lenc(odir))//seed_in(1:lenc(seed_in))//'.swap'
	open(idsc2,file=seed_out,status='unknown',form='unformatted',
     &    access='direct',recl=lenr)
	nfl=lenc(seed_in)
...
c
	...
	if(nrec.gt.1 .and. abs(next_tim-anf_tim).gt.dt*0.5
     &     .and. dt.gt.0) then
...
c
999	istat=ieee_flags('clearall',dummy1,dummy2,dummy3)
	stop
	end
