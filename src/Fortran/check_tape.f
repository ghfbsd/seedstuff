...
c
	...
	open(unit1,file=ofile,status='unknown')
...
c
	...
	do while(istat.ne.4)
	  ...
	  num=0
	  do while (istat.eq.0)
1000	    istat=read_rec_tap(rec_typ,0,unit,unit1,nlong,ndat,lenbo,
...
9	        format(1x,a,i3,3(1x,a),f5.1,1x,2(i2,1h/),
     &            i4,2x,2(i2,1h:),i2,1h.,i4)
	        ...
	        if(abs(new_tim-next_tim(nch,nst)).gt.dt .and.
     &             dt.gt.0.0) then
	        call abstim(0,iftim,iyrf,idyf,ihf,imf,isf)
	        call datum(0,idyf,iyrf,monf,itgf)
...
c	    endif
	  ...
	  if(nfil.ne.nfils) then
	    do nn=1,nfil
	      nch=nchs(nn)
	      call tfix(next_tim(nch,nst),iftim,iftsc)
	      call abstim(0,iftim,iyrf,idyf,ihf,imf,isf)
	      call datum(0,idyf,iyrf,monf,itgf)
...
c
	...
	write(*,*) '  -h <nkb>- skip tape header (<nkb> records, for ',
     &    'RF only)'
