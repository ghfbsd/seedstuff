...
/*                               Prototypes                                 */
/* ======================================================================== */
static inline int   TestIfDir     ( char*, char*, char[] );
...
#define PERROR( x, y )	fprintf( stderr, "\n");\
			strcpy( caErrBuf, x);\
			strcat( caErrBuf, " [" );\
			strcat( caErrBuf, y );\
			strcat( caErrBuf, "] " );\
...
int	FileFindEntrys( char*, char*, char* ),
	...
	else   	/* build a new file and path tree */
	{
		...
		/* ========================================= */
		if( iVerboseFlag ) {
			fprintf( stderr, "\n* path and wildcard call FileFindEntrys( %s, %s, %s )",cpStartDir ,cpSearchPattern, cpWhatSearchFlag );
		}
...
char TmpFlags[255];
	...
	(void)strcpy( TmpFlags, cpFlagStr );
		...
		while( *cpFlagStr )
		{
			switch( *cpFlagStr )
			{
				...
				case '&' :	/* continue with a new      */
					...
					break;
				default :
					PERROR( TmpFlags, "NO SUCH FLAG ERROR" );
					exit( 1 );
			}
			cpFlagStr++;
		}
