 ...
 *  FUNCTION
 *
 *	StrMatch test string for wildcard match
 *
 *  SYNOPSIS
 *
...
BOOLEAN StrMatch (string, pattern)
register char *string;
register char *pattern;
{
...
redo:
	...
	case ASTERISK:
	    pattern++;
	    do {
		ismatch = StrMatch (string, pattern);
...
char *cpGetString = (char*)malloc( SIZE ),
     *cpStr2      = (char*)malloc( SIZE ),
     *cpStr3      = (char*)malloc( SIZE );
	...
	{
		fprintf( stderr, "\nusage : strmatch <SourceString> <Pattern> \n");
		exit(1);
	}
	if( argc == 3 )
	{
 		strcpy( cpStr2, argv[1] );
		strcpy( cpStr3, argv[2] );
		if( StrMatch( cpStr2 ,cpStr3 ) )
			fprintf( fpOutput,"%s\n", cpStr2 );
		else
		{
			fprintf( fpOutput,"%s\n", "" );
	...
	else if( argc == 2)
	{
		...
		while( fgets( cpGetString, SIZE, fpInput ) )
		{
			*(cpGetString+strlen(cpGetString)-1) = '\0';
 			if( StrMatch( cpGetString, cpStr2 ) )
				fprintf( fpOutput,"%s\n", cpGetString );
			else
				fprintf( fpOutput,"\n" );
		}
